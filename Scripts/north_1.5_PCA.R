##################################################################################
## Make PCA of trait data and use to predict seed number
## Daniel Anstett
...
ksr_scale <- ksr_m %>% dplyr::select(Flowering_Date,Water_Content,Growth_Rate,Bolt_Date,SLA,Num_Trichomes,
                              ...
                              Leaf_Oenothein_A,Flower_Oenothein_A,Fruit_Oenothein_A) #select trait data
ksr_scale <- as.data.frame(scale(ksr_scale)) # scale trait data
ksr <- cbind(ksr_m %>% dplyr::select(Pop,Latitude,Longitude,MAT,Seeds),ksr_scale) #merge with meta data
ksr <- na.omit(ksr) #remove columns with NAs to allow for PCA

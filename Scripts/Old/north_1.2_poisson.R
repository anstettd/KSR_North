...
#replace glmmTMB with glmm.zinb: Zero-Inflated Negative Binomial Mixed Models
#https://rdrr.io/github/nyiuab/NBZIMM/man/glmm.zinb.html
...
#1. Does geography predict success?
  qu_geo <- glmmTMB(Seeds ~ Latitude*Longitude + (1|Pop) + (1|Block), data=ksr_i, family=poisson, ziformula=~1)
  qu_geo_main <- glmmTMB(Seeds ~ Latitude+Longitude + (1|Pop) + (1|Block), data=ksr_i, family=poisson, ziformula=~1)
...
#2. Does distance to common garden predict Seeds?
  qu_dist <- glmmTMB(Seeds ~ Distance + (1|Pop) + (1|Block), data=ksr_i, family=poisson, ziformula=~1)

##################################################################################
# Analyses of standardizedlat, climate, and mean trait data predicting seed number
## Daniel Anstett
...
#import data
ksr_m_input <- read.csv("Data/ksr_m.csv", header=T) # Imports individual dataset

#standardize all predictors
ksr_m_response <- ksr_m_input %>% select(Pop,Fruit_no_damage,Seeds) 
ksr_m_std <- ksr_m_input %>% select(Latitude:RH_Distance,Flowering_Date:Fruit_Ox_Oenothein_A) 
ksr_m_std <- scale(ksr_m_std)
ksr_m <- cbind(ksr_m_response,ksr_m_std)
attach(ksr_m)
